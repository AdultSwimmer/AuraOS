...
def main():
    ...
    # Get .txt files sorted by creation time (oldest first)
    files = [
        f for f in os.listdir(RAW_DIR)
        if f.endswith(".txt")
    ]

    files.sort(key=lambda f: os.path.getctime(os.path.join(RAW_DIR, f)))
    ...
    for index, file in enumerate(files, start=1):
        ...
        # Strip .txt, prefix with index number, save as .json
        base_name = file.replace(".txt", "")
        out_name = f"{index}({base_name}).json"
